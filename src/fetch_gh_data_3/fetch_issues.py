...
# New task to fetch a single page with its own retries and backoff
...
@flow
def fetch_issues(repo: str, token: Optional[str] = None, limit: int = 30, exclude_pulls: bool = False, max_pages: int = 20) -> List[Dict[Any, Any]]:
    """
    ...
    for page in range(1, max_pages + 1):
        ...
        try:
            # Fetch this page via dedicated task (handles its own retries)
            page_issues_raw = fetch_issue_page.submit(page_url, headers).result()
        ...
        # Keep original length BEFORE any filtering for pagination checks
        original_count = len(page_issues_raw)
        
        # Filter out pull requests if requested
        if exclude_pulls:
            page_issues = [issue for issue in page_issues_raw if 'pull_request' not in issue]
        else:
            page_issues = page_issues_raw
            
        all_issues.extend(page_issues)
        
        # If the page contained fewer items than requested, we've hit the last page
...
@flow(name="Fetch GitHub Issues", description="Fetch issues from a GitHub repository with automatic retries")
def fetch_gh_issues(repository: str, limit: int = 100, exclude_pulls: bool = True, max_pages: int = 20, token: Optional[str] = None, show_urls: bool = True) -> pd.DataFrame:
    """
    ...
    try:
        # Fetch issues with automatic retries using the task decorator
...
if __name__ == "__main__":
    ...
    repository = "DataDog/datadog-agent"  # Fixed capitalization to match GitHub's format
